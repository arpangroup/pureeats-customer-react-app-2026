...
export const DEMO_ACCOUNTS: { label: string; email: string }[] = [
  { label: 'Demo Customer One', email: 'demo.customer1@pureeats.local' },
  { label: 'Demo Customer Two', email: 'demo.customer2@pureeats.local' },
]
...
export const authService = {
  async register(payload: SignupRequest): Promise<LoginChallengeResponse> {
    if (IS_MOCK) {
      ...
      mockChallenges.set(challengeId, { destination: payload.email, userId: nextMockId(), name: payload.fullName, email: payload.email, phone: '' })
  ...
  async requestOtp(payload: LoginChallengeRequest): Promise<LoginChallengeResponse> {
    if (IS_MOCK) {
      ...
      mockChallenges.set(
        challengeId,
        existing
          ? { destination, userId: existing.id, name: existing.name, email: existing.email, phone: existing.phone }
          : {
              destination,
              userId: nextMockId(),
              name: destination.includes('@') ? destination.split('@')[0] : 'New Customer',
